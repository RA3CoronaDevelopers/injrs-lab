...
use winapi::shared::{minwindef::LPVOID, ntdef::LPCWSTR, windef::HWND};
...
unsafe fn detour(module_name: *const i8, old_func_offset: u64, new_func_address: u64) -> u64 {
    ...
    use winapi::um::{
        ...
        winnt::{
            PAGE_EXECUTE_READWRITE, PIMAGE_DOS_HEADER, PIMAGE_EXPORT_DIRECTORY, PIMAGE_NT_HEADERS,
        },
    ...
    let p_image_export_directory = (module_address
        + (*p_nt_headers).OptionalHeader.DataDirectory[0].VirtualAddress as u64)
...
unsafe fn test_message_box_w_hook() -> i32 {
    ...
    (*(&GetProcAddress(
        GetModuleHandleA("USER32.dll\0".as_ptr() as _),
...
pub extern "system" fn DllMain(
    ...
    match reason {
        1 => unsafe {
            CreateThread(null_mut(), 0, Some(init_hook), null_mut(), 0, null_mut());
        },
        0 => (),
        _ => (),
    }
